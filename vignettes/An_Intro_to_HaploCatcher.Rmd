...
# Example Datasets Included in HaploCatcher
...
There are three different data frames included in the HaploCatcher package; "geno_mat", "gene_comp", and "marker_info". 
...
In this example, only individuals who are in the gene_comp are found in the geno_mat file provided in the package, so we will have to randomly partition the data so that we can show how this can be used for forward prediction!
...
In this case we assume that the individuals in the "testing_genotypes" vector **DO NOT** have *Sst1* data associated with them and we are trying to produce a predictive haplotype for these individuals.
...
- geno_mat is the genotypic matrix which contains marker information
- gene_file is the gene compendium file that contains information about the *Sst1* locus
- gene_name is a character string which defines the name of the gene to be predicted found in the "gene_file" input
- marker_info is the marker information file that contains all the chromosome and position information about the markers in the "geno_mat" file
- chromosome is the chromosome that the gene resides upon
- training_genotypes is a vector of FullSampleNames from the geno_mat row names that defines which individuals are to be used for training of the KNN or RF models
- testing_genotypes is a vector of FullSampleNames from the geno_mat row names which are to be predicted
- set_seed is a number used to set the seed of the function. This option must be defined if the "predict_by_vote" function is set to FALSE, which is the default setting.
...
Furthermore, there are options for sequential modeling (parallel=FALSE) or parallel modeling (parallel=TRUE), as well as methods to suppress output of text (verbose=FALSE). To save time, we will process one model in parallel (parallel=TRUE) and the other sequentially (parallel=FALSE) to see the output. Sequential processing is the default. 

The example data set for the HaploCatcher package contains heterozygous calls, so we will perform both models using the "include_hets" argument. We will set n_perms=10 for expediency.  
...
```{r, example_models_2, fig.width=12, fig.height=8, fig.align='center', out.width="100%"}
...
Each result obtained puts out a list-of-list objects which contain. 
...
Regardless if the "include_hets" argument is set to either TRUE or FALSE, the results should be found in results_object$predictions.
...
```{r, predict_by_vote_model, fig.width=12, fig.height=8, fig.align='center', out.width="100%"}
#prediction by voting
...
Note that the predictions, when made through voting, may be found in the results$consensus_predictions pathway. This data frame shows the FullSampleName, a haplotype call titled "Consensus_Call" made by majority rule, and the different categories with a number of times the line was identified as that haplotype.
...
Each results object has cross validation parameters to check the predictive ability of the data set. To see this, users can access summaries in the list-of-list results object.
...
Here we showed a short tutorial of how to use the HaploCatcher package. We hoped this was informative for potential users and that this fuels interest in the use of our package. Thank you. 
